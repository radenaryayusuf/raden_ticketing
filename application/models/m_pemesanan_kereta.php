<?php 
class m_pemesanan_kereta extends CI_Model{
    var $tabel = 'stasiun';
	function __construct(){
		parent:: __construct();
	}
	
        //  function get_koderute(){
        //     $result = $this->db->select('rute_id')->from('rute')->where('rute_from', $rute_from,'And rute_to', $rute_to)->limit(1)->get()->row();
        // $id_rute =  $result->rute_id;
...
function korsi_sisa($rute_id){
    $date_now =  date("Ymd");
     $time_now =  date("H:i:s");
       $query = $this->db->query(
    "SELECT transportation.seat_qty - COUNT(reservation.rute_id) FROM reservation inner join rute on reservation.rute_id = rute.rute_id inner join transportation on rute.transportation_id = transportation.transportation_id Where reservation.rute_id = '{$rute_id}' AND SUBSTRING(reservation.reservation_id, 1, 8) = '$date_now'  AND rute.depart_to >= '$time_now'")->result_array();
...
function prosesorder(){
    ...
    function get_rute_by_ruteid($Rute_id){
        ...
        function kereta_list($rute_from, $rute_to){
            $transportation_type_id = 'TRID0002';
        ...
        return $this->db->query("SELECT distinct(st.city),st.name,rt.rute_from,st.abbr from stasiun st inner join rute rt on st.id = rt.rute_from");
            }
    function  get_ruteto() {  //funtion menampilkan semua data
        return $this->db->query("SELECT distinct(st.city),st.name, rt.rute_to,st.abbr from stasiun st inner join rute rt on st.id = rt.rute_to");
           }
          function get_name1($rute_from){
             $result = $this->db->select('name')->from('stasiun')->where('id', $rute_from)->limit(1)->get()->row();
        $nama_stasiun =  $result->name;

        return $nama_stasiun;
           }
         function get_name2($rute_to){
           $result = $this->db->select('name')->from('stasiun')->where('id', $rute_to)->limit(1)->get()->row();
        $nama_stasiun =  $result->name;
        ...
        $this->db->select('st.city');
        $this->db->from("stasiun as st");
        $this->db->join('rute as rt', 'rt.rute_from = st.id');
