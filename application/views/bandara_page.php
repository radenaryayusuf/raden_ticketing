        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title" id="myModalLabel">Form Data Bandara</h4>
                </div>
                <div class="modal-body">
                    <form role="form" class="insert_bandara" id="formvalidationtooltip">
                                    <div class="form-body">

                                        <div class="form-group">
                                            <label>ID</label>
                                           <input type="text" class="form-control validate[required]" name="id" id="id" placeholder="ID Bandara" data-prompt-position="topLeft" readonly>
                                        </div>
     ...
     <div class="modal modal-secondary fade" id="ModalaEdit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title" id="myModalLabel">Form Edit Bandara</h4>
                </div>
                <div class="modal-body">
                    <form action="#" role="form" class="update_bandara" id="formvalidationtooltip">
             <div class="form-body">

                                        <div class="form-group">
                                            <label>ID</label>
                                           <input type="text" class="form-control validate[required]" name="id_edit" id="id2" placeholder="ID Bandara" data-prompt-position="topLeft" readonly>
                                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                 <input type="hidden" name="kode" id="textkode" value="">
                  <div class="alert alert-warning"><p>Apakah Anda yakin mau menghapus bandara ini?</p></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
    ...
    <div class="modal modal-secondary fade" id="ModalConfirInsert" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="alert alert-warning"><p>Apakah Anda yakin mau menambahkan bandara ini?</p></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
    ...
    <div class="modal modal-secondary fade" id="ModalConfirUpdate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                  <div class="alert alert-warning"><p>Apakah Anda yakin mau mengupdate bandara ini?</p></div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
...
function tampil_data_bandara(){
            $.ajax({
                type  : 'ajax',
                url   : '<?php echo base_url()?>index.php/Databandara/data_bandara',
                async : false,
                dataType : 'json',
                    ...
                    for(i=0; i<data.length; i++){
                        html += '<tr>'+
                                ...
                                '<td style="text-align:center">'+
                                    '<a href="javascript:;" class="btn btn-info btn-md item_edit" data="'+data[i].bandara_id+'">Edit</a>'+' | '+
                                    '<a href="javascript:;" class="btn btn-danger btn-md item_hapus" data="'+data[i].bandara_id+'">Hapus</a>'+
                                '</td>'+
                                '</tr>';
                    }
                    $('#show_data').html(html);
        ...
        $('#btn_simpan').on('click',function(){
            ...
            var city = $('#city').val();
            var abbr = $('#abbr').val();
            var datanya = $('.insert_bandara').serializeArray();
            $.ajax({
                type : "POST",
                url  : "<?php echo site_url('Databandara/simpan_bandara')?>",
                dataType : "JSON",
                data : datanya,
                success: function(data){
                    $('[name="id"]').val("");
        ...
        $('#show_data').on('click','.item_edit',function(){
            ...
            $.ajax({
                type : "GET",
                url  : "<?php echo base_url('index.php/Databandara/get_bandara')?>",
                dataType : "JSON",
                data : {id:id},
                success: function(data){
                    $.each(data,function(bandara_id, name, city, abbr){
                        $('#ModalaEdit').modal();
                        $('[name="id_edit"]').val(data.bandara_id);
        ...
        $('#btn_update').on('click',function(){
           var id = $('#id2').val();
            ...
            var city = $('#city2').val();
            var abbr = $('#abbr2').val();
            var datanyaa = $('.update_bandara').serializeArray();
            $.ajax({
                type : "POST",
                url  : "<?php echo site_url('Databandara/update_bandara')?>",
                dataType : "JSON",
                data : datanyaa,
                success: function(data){
                    $('[name="id_edit"]').val("");
        ...
        $('#btn_hapus').on('click',function(){
            ...
            $.ajax({
            type : "POST",
            url  : "<?php echo base_url('index.php/Databandara/hapus_bandara')?>",
            dataType : "JSON",
                    data : {kode: kode},
                    success: function(data){
        ...
        $('#abbr').keyup(function(){
    var abbr = $('#abbr').val();
    var url = "<?php echo base_url()  ?>index.php/Databandara/kodeBandara/"+abbr;
    $.get(url,function(e){
      $('#id').val(e);
    })
  });
